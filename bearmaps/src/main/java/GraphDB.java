...
public class GraphDB {
    ...
    public GraphDB(String dbPath) {
        ...
        clean();
        kdTree = new KDTree(vertexHashMap.values());
    }
    ...
    public static class Vertex {
        ...
        double x;
        double y;
        HashMap<Long, Double> adjacent;
        long nodeID;

        Vertex(long id, double longi, double lati) {
            this.lon = longi;
            this.lat = lati;
            this.x = projectToX(longi, lati);
            this.y = projectToY(longi, lati);
            this.nodeID = id;
            adjacent = new HashMap<>();
        }
    }

    public void addVertex(long id, double longi, double lati) {
        Vertex v = new Vertex(id, longi, lati);
        vertexHashMap.put(id, v);
    }

    public void addAdjacent(ArrayList<Long> adjacents) {
        for (int i = 0; i < adjacents.size() - 1; i++) {
            long firstID = adjacents.get(i);
            long secondID = adjacents.get(i + 1);
            vertexHashMap.get(firstID).adjacent.put(secondID, distance(secondID, firstID));
            vertexHashMap.get(secondID).adjacent.put(firstID, distance(firstID, secondID));
        }
    }
     ...
     */
    private void clean() {
        ArrayList<Long> verticesToRemove = new ArrayList<>();
        for (Vertex v : vertexHashMap.values()) {
            if (v.adjacent.size() == 0) {
                verticesToRemove.add(v.nodeID);
            }
        }
        for (long id : verticesToRemove) {
            vertexHashMap.remove(id);
        }
    }
    ...
    Iterable<Long> adjacent(long v) {
        return vertexHashMap.get(v).adjacent.keySet();
        //return Collections.emptySet();
    }
    ...
    public long closest(double lon, double lat) {
        ...
        KDTree.Node toReturn =  kdTree.nearestNeighbour(x, y, 0, kdTree.root, kdTree.root);

        return toReturn.id;
/*       long temp = -999;
        vertexHashMap.put(temp, new Vertex(temp, lon, lat));
        long toReturn = temp;
        double smalld = Double.MAX_VALUE;
        for (Vertex v : vertexHashMap.values()) {
            if (v.nodeID != temp) {
                double dist = distance(v.nodeID, temp);
                if (dist < smalld) {
                    smalld = dist;
                    toReturn = v.nodeID;
                }
