...
public class Router {
    ...
    public static List<Long> shortestPath(GraphDB g,
        ...
        //Initialise sets for visited, distance and parent sets
        HashSet<Long> visited = new HashSet<>();
        int vertexCount = g.vertexHashMap.size();
        HashMap<Long, Double> distance = new HashMap<>();
        //HashMap<Long, Double> heuristics = new HashMap<>();
        HashMap<Long, Long> parent = new HashMap<>();
        LinkedList<Long> pathList = new LinkedList<>();

        //Initiate priority queue
        PriorityQueue<Long> fringe = new PriorityQueue<>(vertexCount,
                ((o1, o2) -> (Double.compare(distance.get(o1) + g.distance(o1, stop),
                        distance.get(o2) + g.distance(o2, stop)))));
        ...
        while (!fringe.isEmpty()) {
            ...
            for (Long w : g.adjacent(v)) {
                if (visited.contains(w)) {
                    continue;
                } else if (!fringe.contains(w)) {
                    double pathDist = g.distance(v, w);
                    double prevDist = distance.get(v);
                    //double preHeu = g.distance(w, stop);
                    parent.put(w, v);
                    distance.put(w, prevDist + pathDist);
                    //heuristics.put(w, preHeu);
                    fringe.add(w);
                } else {
                    double pathDist = g.distance(v, w);
                    double prevDist = distance.get(v);
                    if ((prevDist + pathDist) < distance.get(w)) {
                        parent.put(w, v);
                        distance.put(w, prevDist + pathDist);
                        fringe.add(w);
                    }
        ...
        for (long path = stop; path != start; path = parent.get(path)) {
